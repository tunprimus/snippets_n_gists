...
def extract_text_with_pymupdf(pdf_path, output_path="extracted_text.txt"):
    real_path_to_pdf = realpath(pdf_path)
    real_path_to_output = realpath(output_path)
    doc = fitz.open(real_path_to_pdf)
    text = "\n".join([page.get_text("text") for page in doc])
    with open(real_path_to_output, "w", encoding="utf-8") as fop:
        fop.write(text)
...
def extract_tables_with_tabula(pdf_path, output_path="extracted_tables.csv"):
    real_path_to_pdf = realpath(pdf_path)
    real_path_to_output = realpath(output_path)
    try:
        dfs = tabula.read_pdf(real_path_to_pdf, pages="all", multiple_tables=True)
        for i, df in enumerate(dfs):
            df.to_csv(f"{real_path_to_output[:-4]}_{i}.csv", index=False)
...
def extract_tables_with_pdfplumber(pdf_path, output_path="extracted_tables.csv"):
    real_path_to_pdf = realpath(pdf_path)
    real_path_to_output = realpath(output_path)
    try:
...
def extract_tables_with_camelot(pdf_path, output_path="extracted_tables.csv"):
    real_path_to_pdf = realpath(pdf_path)
    real_path_to_output = realpath(output_path)
    try:
        tables = camelot.read_pdf(real_path_to_pdf, pages="1", flavor="lattice")
        for i, table in enumerate(tables):
            table.df.to_csv(f"{real_path_to_output[:-4]}_{i}.csv", index=False)
...
def extract_text_with_tesseract(pdf_path, output_path="ocr_extracted_text.txt"):
    real_path_to_pdf = realpath(pdf_path)
    real_path_to_output = realpath(output_path)
...
def extract_images_with_pymupdf(pdf_path, output_path="extracted_image.png"):
    real_path_to_pdf = realpath(pdf_path)
    real_path_to_output = realpath(output_path)
    doc = fitz.open(real_path_to_pdf)
...
def auto_extract_pdf_data(pdf_path, output_path="."):
    ...
    if is_pdf_text_based(real_path_to_pdf):
        ...
        extract_text_with_pymupdf(real_path_to_pdf, real_path_to_output)
        if not extract_tables_with_tabula(real_path_to_pdf, real_path_to_output):
            if not extract_tables_with_pdfplumber(real_path_to_pdf, real_path_to_output):
                extract_tables_with_camelot(real_path_to_pdf, real_path_to_output)
    else:
        print("[INFO] PDF appears to be scanned. Using OCR...")
        extract_text_with_tesseract(real_path_to_pdf, real_path_to_output)

    # Extract images
    extract_images_with_pymupdf(real_path_to_pdf, real_path_to_output)
